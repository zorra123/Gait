...
void walking::process() {
    ...
    if (m_time <= m_ssp_time_end_l) {
        m_body_swing_y = -ep[L_HIP_ROLL];
        m_body_swing_z = ep[L_HIP_PITCH];
    } else {
        m_body_swing_y = -ep[R_HIP_ROLL];
        m_body_swing_z = ep[R_HIP_PITCH];
    }
    ...
    // Compute motor value
    angle[R_HIP_ROLL] += pelvis_offset_r;
...
//         TODO Read from message
        ...
        // TODO TUNE IT!
        angle[R_HIP_ROLL] += dir[R_HIP_ROLL] * rl_gyro_err * balance_hip_roll_gain * 4;
        angle[L_HIP_ROLL] += dir[L_HIP_ROLL] * rl_gyro_err * balance_hip_roll_gain * 4;
        angle[R_KNEE] -= dir[R_KNEE] * fb_gyro_err * balance_knee_gain * 4;
        angle[L_KNEE] -= dir[L_KNEE] * fb_gyro_err * balance_knee_gain * 4;
        angle[R_ANKLE_PITCH] -= dir[R_ANKLE_PITCH] * fb_gyro_err * balance_ankle_pitch_gain * 4;
        angle[L_ANKLE_PITCH] -= dir[L_ANKLE_PITCH] * fb_gyro_err * balance_ankle_pitch_gain * 4;
        angle[R_ANKLE_ROLL] -= dir[R_ANKLE_ROLL] * rl_gyro_err * balance_ankle_roll_gain * 4;
        angle[L_ANKLE_ROLL] -= dir[L_ANKLE_ROLL] * rl_gyro_err * balance_ankle_roll_gain * 4;
    }
