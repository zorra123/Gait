...
*/
    ...
    BALANCE_ENABLE = true;
/* todo SetAngle */
    SetAngle(ID_R_SHOULDER_PITCH, -48.345);
    SetAngle(ID_L_SHOULDER_PITCH, 41.313);
    SetAngle(ID_R_SHOULDER_ROLL, -17.873);
    SetAngle(ID_L_SHOULDER_ROLL, 17.580);
    SetAngle(ID_R_ELBOW, 29.300);
    SetAngle(ID_L_ELBOW, -29.593);

    SetAngle(ID_HEAD_TILT, Kinematics::EYE_TILT_OFFSET_ANGLE);

/* todo SetPGain*/
    SetPGain(ID_R_SHOULDER_PITCH, 8);
    SetPGain(ID_L_SHOULDER_PITCH, 8);
    SetPGain(ID_R_SHOULDER_ROLL, 8);
    SetPGain(ID_L_SHOULDER_ROLL, 8);
    SetPGain(ID_R_ELBOW, 8);
    SetPGain(ID_L_ELBOW, 8);
...
/* todo SetValue */
    ...
    SetValue(ID_R_HIP_PITCH,         outValue[2]);
    SetValue(ID_R_KNEE,              outValue[3]);
    SetValue(ID_R_ANKLE_PITCH,       outValue[4]);
    SetValue(ID_R_ANKLE_ROLL,        outValue[5]);
    SetValue(ID_L_HIP_YAW,           outValue[6]);
    SetValue(ID_L_HIP_ROLL,          outValue[7]);
    SetValue(ID_L_HIP_PITCH,         outValue[8]);
    SetValue(ID_L_KNEE,              outValue[9]);
    SetValue(ID_L_ANKLE_PITCH,       outValue[10]);
    SetValue(ID_L_ANKLE_ROLL,        outValue[11]);
    SetValue(ID_R_SHOULDER_PITCH,    outValue[12]);
    SetValue(ID_L_SHOULDER_PITCH,    outValue[13]);
    SetAngle(ID_HEAD_PAN, A_MOVE_AMPLITUDE);
